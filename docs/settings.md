...
| `retry.modelFallback`                    | boolean | `true`            | Fall back to another model when one is unavailable. Also used by online auto-thinking classification and session titles (tiny/smol/commit). When `false`, session titles only attempt their first resolvable role model, without hopping to other roles or configured fallback models; the typed-judge chain (auto-thinking, unexpected-stop, AI staging) still visits every role but skips configured fallback chains.                                                                                                                                                                                                                                                          |
