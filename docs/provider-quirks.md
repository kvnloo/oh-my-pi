...
- **Synthetic Terminal Boundaries**: If the SSE stream closes without a `done` or `error` event, `streamPiNative` closes the stream deterministically so `.result()` cannot hang. Caller cancellation synthesizes an assistant message via `makeSyntheticAssistant` and pushes `{ type: "error", reason: "aborted", error: { ..., stopReason: "aborted", errorMessage: "stream closed without terminal event" } }`; any other clean close fails the stream with `AIError.ProviderResponseError` (`"pi-native stream read error: stream closed before a terminal response event"`, `kind: "incomplete-stream"`) so the turn is retried instead of resolving to an empty response.
