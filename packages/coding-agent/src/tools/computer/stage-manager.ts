...
function parseMonitorGeometry(value: unknown): HyprlandMonitorGeometry | null {
	...
	const reserved = [0, 1, 2, 3].map(index => Math.round(numberValue(reservedRaw[index]) / scale)) as [
		number,
		number,
		number,
		number,
...
export function computeCarouselLayout(options: {
	...
	const visibleAddresses = [left?.address, activeAddress, right?.address].filter((address): address is string =>
		Boolean(address),
	);
...
export class HyprlandStageManager {
	...
	async execGated(shellCommand: string, signal?: AbortSignal): Promise<{ expression: string }> {
		...
		if (!target) {
			throw new ToolError("execGated requires OMP_E2E_MONITOR and OMP_E2E_WORKSPACE (handsfree E2E isolation)");
		}
	...
	async #activateCarousel(stage: HyprlandStage, activeAddress: string, signal?: AbortSignal): Promise<void> {
		...
		const monitor =
			...
			snapshot.monitors.find(
				candidate => candidate.id === snapshot.clients.find(c => c.address === activeAddress)?.monitor,
			) ??
			snapshot.monitors[0];
