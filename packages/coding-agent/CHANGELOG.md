...
- Widened the `cycle_thinking_level` RPC response `data.level` from `Effort` to `ConfiguredThinkingLevel` so the declared wire contract matches the `"off"` and `"auto"` selectors the server already emits.
