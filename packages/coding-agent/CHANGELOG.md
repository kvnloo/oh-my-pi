...
- Mojeek web search now maps the `week` recency filter to a `YYYYMMDD` date 7 days ago instead of forwarding an undocumented `since=week` token, so a `week` time filter is reliably applied ([{{detail_github_issue_ref}}]({{detail_github_issue_url}})).
