...
- Fixed aborted or timed-out browser drags leaving the mouse button pressed, which broke later `tab.click`/`tab.drag` calls on the same worker ([{{detail_github_issue_ref}}]({{detail_github_issue_url}})).
